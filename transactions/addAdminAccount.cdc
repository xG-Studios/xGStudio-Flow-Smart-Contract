...
transaction(admin: Address) {
    prepare(signer: AuthAccount) {
       ...
       // get the public capability from the Admin's public storage
        let AdminResource = TemplateAdminAccount.getCapability
            <&{xGStudios.UserSpecialCapability}>
        ...
        //get admin refrence for adding AdminCapability
         let adminRef = signer.getCapability<&xGStudios.AdminCapability>(xGStudios.AdminCapabilityPrivate).borrow() 
        ...
        if specialCapability.check() {
            ...
            AdminResource.addCapability(cap: specialCapability)
